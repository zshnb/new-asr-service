...
from lib.faster_whisper.utils import get_assets_path
...
def get_speech_timestamps(
    ...
    **kwargs,
    ...
    padded_audio = np.pad(audio.numpy(), (0, window_size_samples - audio.shape[0] % window_size_samples))
    ...
    if current_speech:
        if not temp_end:
            temp_end = audio_length_samples
        if temp_end - current_speech['start'] > min_speech_samples:
            current_speech["end"] = temp_end
            speeches.append(current_speech)
...
class SpeechTimestampsMap:
    ...
    def __init__(self, chunks: List[dict], sampling_rate: int, time_precision: int = 2):
        ...
        for chunk in chunks:
            ...
            self.total_silence_before.append(silent_samples / self.sampling_rate)
    ...
    def get_original_time(
        ...
        if chunk_index is None:
            ...
            if start_or_end == 'start':
                if (self.chunk_end_sample[chunk_index] / self.sampling_rate) - time < 0.2:
                    chunk_index += 1
...
class SileroVADModel:
    ...
    def __call__(self, audio: np.ndarray, num_samples: int = 512, context_size_samples: int = 64):
        assert audio.ndim == 2, "Input should be a 2D tensor with size (batch_size, num_samples)"
