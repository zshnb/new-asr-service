...
from lib.faster_whisper.audio import decode_audio, pad_or_trim
from lib.faster_whisper.feature_extractor import FeatureExtractor
from lib.faster_whisper.tokenizer import _LANGUAGE_CODES, Tokenizer
from lib.faster_whisper.utils import download_model, format_timestamp, get_end, get_logger
from lib.faster_whisper.vad import (
    SpeechTimestampsMap,
    VadOptions,
    collect_chunks,
...
class BatchedInferencePipeline:
    ...
    def forward(self, features, chunks_metadata, **forward_params):
        encoder_output, outputs = self.model.generate_segment_batched(features, self.tokenizer, forward_params)
            ...
            segmented_outputs.append(
                [
                    dict(
                        ...
                        compression_ratio=get_compression_ratio(self.tokenizer.decode(subsegment["tokens"])),
                    )
    ...
    def get_language_and_tokenizer(self, audio, task: Optional[str] = None, language: Optional[str] = None):
    ...
    def transcribe(
        ...
        if self.model.model.is_multilingual:
            ...
            if language is not None:
                self.model.logger.warning(
                    f"English-only model is used, but {language} language is chosen, setting language to 'en'."
                )
            language = "en"
        ...
        to_cpu = self.model.model.device == "cuda" and len(self.model.model.device_index) > 1
        features = (
            torch.stack(
                [
                    pad_or_trim(
    ...
    def _batched_segments_generator(self, features, chunks_metadata, batch_size, options, log_progress):
...
class WhisperModel:
    def __init__(
        ...
        **model_kwargs,
        ...
        self.feature_extractor = FeatureExtractor(**self.feat_kwargs, device=self.device)
        self.input_stride = 2
    ...
    def transcribe(
        ...
        initial_prompt: Optional[Union[str, Iterable[int], dict]] = None,
        prefix: Optional[str] = None,
        suppress_blank: bool = True,
        suppress_tokens: Optional[List[int]] = [-1],
        without_timestamps: bool = False,
        max_initial_timestamp: float = 1.0,
        word_timestamps_dict: dict = {'zh': True},
        prepend_punctuations: str = "\"'“¿([{-",
        append_punctuations: str = "\"'.。,，!！?？:：”)]}、",
        ...
        features = self.feature_extractor(audio, chunk_length=chunk_length, to_cpu=to_cpu)
        ...
        if language is None:
            if not self.model.is_multilingual:
                ...
                if duration_after_vad > 90:
                    segment_duration = self.feature_extractor.nb_max_frames
                    total_frames = features.shape[1]
                    segment_length = min(segment_duration, total_frames)

                    # 截取开始部分（前30秒）
                    start_segment = features[:, 0:segment_length]

                    # 截取中间部分（中间30秒）
                    mid_start = (total_frames - segment_length) // 2
                    middle_segment = features[:, mid_start : mid_start + segment_length]

                    # 截取结束部分（最后30秒）
                    end_segment = features[:, -segment_length:]
                    languages = []
                    for seg in [end_segment, middle_segment, start_segment]:
                        encoder_output = self.encode(pad_or_trim(seg))
                        results = self.model.detect_language(encoder_output)[0]
                        all_language_probs = [(token[2:-2], prob) for (token, prob) in results]
                        language, language_probability = all_language_probs[0]
                        languages.append((language, language_probability))
                    first_elements = [t[0] for t in languages if t[1] >= 0.85]
                    seconde_elements = [t[1] for t in languages if t[1] >= 0.85]
                    if len(set(first_elements)) == 1:
                        language, language_probability = first_elements[0], max(seconde_elements)
                        info_language = language
                    else:
                        info_language = 'multi'
                    ...
                    start_timestamp = (
                        float(clip_timestamps.split(",")[0]) if isinstance(clip_timestamps, str) else clip_timestamps[0]
                    )
                    ...
                    while seek <= end_frames:
                        ...
                        language, language_probability = all_language_probs[0]
                        if language_detection_threshold is None or language_probability > language_detection_threshold:
                            break
            ...
            self.logger.info(
                "Detected language '%s' with probability %.2f, but info_language: '%s' ",
                language,
                language_probability,
                info_language,
            )
        ...
        specific_init_prompt = None
        word_timestamps = True
        if word_timestamps_dict:
            default_value = word_timestamps_dict.get('default', True)
            word_timestamps = word_timestamps_dict.get(language, default_value)
        if initial_prompt:
            specific_init_prompt = initial_prompt.get(language, None)
        tokenizer = Tokenizer(
            self.hf_tokenizer,
        ...
        options = TranscriptionOptions(
            ...
            temperatures=(temperature if isinstance(temperature, (list, tuple)) else [temperature]),
            initial_prompt=specific_init_prompt,
            prefix=prefix,
            suppress_blank=suppress_blank,
        ...
        # debug
        duration_debug = audio.shape[0] / sampling_rate
        self.logger.info(f"Real audio duration_debug: {format_timestamp(duration_debug)}")
    ...
    def generate_segments(
        ...
        while clip_idx < len(seek_clips):
            ...
            if options.word_timestamps:
                ...
                current_segments = self.remove_unvalid_segments(current_segments)
    ...
    def remove_unvalid_segments(self, segments: list[dict]):
        new_segments = []
        for segment in segments:
            words = segment.get('words', None)
            if not words or all((w.get('word', '') == "" for w in words)):
                continue
            else:
    ...
    def add_word_timestamps(
        ...
        for segment_idx, segment in enumerate(segments):
            ...
            has_med_max_dura = False
            if segment_idx < len(median_max_durations):
                median_duration, max_duration = median_max_durations[segment_idx]
                has_med_max_dura = True
            else:
                has_med_max_dura = False
    ...
    def detect_language(self, audio: torch.Tensor):
        to_cpu = self.model.device == "cuda" and len(self.model.device_index) > 1
        segment = self.feature_extractor(audio, padding=True, to_cpu=to_cpu)[:, : self.feature_extractor.nb_max_frames]
        encoder_output = self.encode(pad_or_trim(segment))
        results = self.model.detect_language(encoder_output)
        language_token, language_probability = results[0][0]
        language = language_token[2:-2]
        self.logger.info(f"Detected language: {language} ({language_probability:.2f}) in first 30s of audio...")
        all_language_probs = [(token[2:-2], prob) for (token, prob) in results[0]]
        return language, language_probability, all_language_probs

    def detect_language_multi_segment(self, audio: Union[str, BinaryIO, torch.Tensor], params: Optional[dict] = None):
        """
        ...
        if params.get("multilingual", False):
            logging.warning("lang_id is not supported for multilingual audios, detecting the major language.")
        ...
        if num_detection_segments > num_segments:
            logging.warning(f"Lang ID: Can not have more segments, setting {num_segments} segments.")
        ...
        if detected_languages:
            ...
            is_silent = torch.all(audio.abs() < 0.01) or torch.sqrt(torch.mean(audio_minus_dc_offset**2)) < 0.01
    ...
    for segment in segments:
        if segment.words:
            ...
            for word in segment.words:
                ...
                chunk_index = ts_map.get_chunk_index(word.end)
            ...
            segment.end = ts_map.get_original_time(segment.end, start_or_end='end')
